...
public:
    ...
    inline dataformats::raw_fed::fed_header const& get_fed_header() const { return fedheader; }
    ...
    */
    ...
    uint64_t const* payload(int imod) const {
        uint64_t const *ptr = reinterpret_cast<uint64_t const*>(&fedheader) + 2 + namc();
...
private:
    dataformats::raw_fed::fed_header fedheader;
